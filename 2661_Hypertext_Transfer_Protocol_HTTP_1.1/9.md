...
HTTP/1.1的实现者应该注意到的一点是，在用户与资源的交互过程中，软件实际上扮演的是用户的角色。因此，在实现那些可能会使用户意识到他们的行为可能会给其他用户或资源产生额外影响时，应该仔细慎重。

具体来说，约定俗成的规则是，除了获取数据外，**GET**和 **HEAD**方法 **不应该（SHOULD NOT）**产生其他影响，这两个方法也被认为是 **安全的（safe）**。按照这个规则，用户端代理就可以区别对待其他方法（如 **POST** **PUT** **DELETE**），以便使用户意识到其正在发送的可能一个是不安全的请求。
