...
响应码410表示，服务器已经无法获取到被请求的资源了，而且也不知该到哪里去找，这种情况应被认为是永久性的。而无论这种情况是不是永久性的，具有编辑能力的客户端 **应该（SHOULD）**在获得用户许可后删除指向该Request-URI的应用。如果服务器不知道，或者没法确定这种情况是不是永久性的，则服务器 **应该（SHOULD）**返回响应码404.除非特别指明，否则是可以缓存该相应结果的。
